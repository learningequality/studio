...
var SettingsView = BaseViews.BaseListEditableItemView.extend({
  ...
  render: function() {
    ...
    $('#mastery_custom_criterion').css(
      'visibility',
      $('#mastery_model_select').val() === 'm_of_n' ? 'visible' : 'hidden'
    );
  ...
  get_license_id: function(license_name) {
    return license_name && license_name != 'None'
      ? Constants.Licenses.find(license => license.license_name === license_name).id
      : 0;
  },
  get_license_name: function() {
    ...
    return (
      el.val() != 0 &&
      Constants.Licenses.find(license => license.id === parseInt(el.val(), 10)).license_name
    );
  },
  check_custom_license: function() {
    ...
    return (
      el.val() != 0 &&
      Constants.Licenses.find(license => license.id === parseInt(el.val(), 10)).is_custom
    );
  },
  submit_changes: function() {
    var content_defaults = this.model.get('content_defaults');
    content_defaults =
      typeof content_defaults === 'string' ? JSON.parse(content_defaults) : content_defaults;
    content_defaults.license = this.get_license_name();
    content_defaults.author = $('#author_field')
      .val()
      .trim();
    content_defaults.aggregator = $('#aggregator_field')
      .val()
      .trim();
    content_defaults.provider = $('#provider_field')
      .val()
      .trim();
    content_defaults.copyright_holder = $('#input_copyright_holder')
      .val()
      .trim();
    content_defaults.license_description = $('#custom_license_description')
      .val()
      .trim();
    ...
    var self = this;
    $('#settings_submit')
      .html(this.get_translation('saving'))
