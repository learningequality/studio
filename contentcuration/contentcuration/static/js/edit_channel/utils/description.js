  ...
  */
  ...
  el.html(
    this.template({
      ...
      overflow:
        description.length > split_index
          ? description.substring(split_index, description.length)
          : null,
  ...
  this.toggle = function(event) {
    ...
    el.find('.toggle_description').text(
      this.expanded ? stringHelper.translate('more') : stringHelper.translate('less')
    );
    this.expanded
      ? el.find('.overflow').slideUp(100)
      : el.find('.overflow').slideDown(100, function() {
          el.find('.overflow').css('display', 'inline');
        });
