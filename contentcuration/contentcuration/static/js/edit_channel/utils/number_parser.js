...
var INTEGER = new RegExp(
  '(' + SIGN.source + '(?:' + FORMATTED_INT.source + '|' + UNFORMATTED_INT.source + '))'
);
var NON_ZERO_INT = new RegExp(
  '(' +
    SIGN.source +
    NON_ZERO_DIGIT.source +
    '(?:' +
    DIGIT.source +
    '{0,2}(?:' +
    SEP.source +
...
var EXPONENT = new RegExp(
  '(' + DECIMAL.source + '|' + INTEGER.source + ')e\\+?(' + INTEGER.source + ')'
);
...
function test_valid_number(text) {
  return (
    test_exponent(text) ||
    test_mixed_number(text) ||
    test_fraction(text) ||
    test_percentage(text) ||
    test_decimal(text) ||
    test_integer(text)
  );
}
