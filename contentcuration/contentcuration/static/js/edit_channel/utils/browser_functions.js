...
function get_max_parallel_uploads() {
  ...
  switch (browser_details.name.toLowerCase()) {
    case 'opera':
      if (browser_details.version < 10) {
        return 4;
      } else if (browser_details.version === 10) {
        return 8;
      } else {
        return 6;
      }
    case 'chrome':
      if (browser_details.version === 3) {
        return 4;
      } else {
        return 6;
      }
    case 'safari':
      if (browser_details.version >= 3 && browser_details <= 4) {
        return 4;
      } else {
        return 2;
      }
