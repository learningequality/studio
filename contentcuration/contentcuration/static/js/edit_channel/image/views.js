...
const THUMBNAIL_CROP_BOUNDARY = {
  ...
  height: THUMBNAIL_ASPECT_RATIO.height + 10,
};
...
export const ThumbnailUploadView = BaseViews.BaseView.extend({
  ...
  initialize: function(options) {
    ...
    this.image = _.find(this.model.get('files'), function(f) {
  ...
  remove_image: function() {
    var self = this;
    dialog.dialog(
      this.get_translation('removing_image'),
      this.get_translation('removing_image_text'),
      {
  ...
  submit_image: function() {
    ...
    if (this.onsuccess) {
      this.onsuccess(
        this.image,
        this.thumbnail_encoding,
        this.image_formatted_name,
        this.image_url
      );
    }
    if (this.onfinish) {
    ...
    this.node
      ...
      .catch(function(error) {
        ...
        self
          ...
          .text(
            error.status === 403
              ? self.get_translation('no_space')
              : self.get_translation('unable_to_generate')
          );
        self.enable_generate();
      });
  },
...
export const ImageUploadView = BaseViews.BaseModalView.extend({
  ...
  render_dropzone: function() {
    this.$('.modal-body').html(
      this.template({ file: this.file, alt_text: this.alt_text }, { data: this.get_intl_data() })
    );
    Dropzone.autoDiscover = false;
