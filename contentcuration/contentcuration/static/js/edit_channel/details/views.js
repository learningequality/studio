...
var MESSAGES = {
  ...
  delete_warning:
    'All content under this channel will be permanently deleted.\nAre you sure you want to delete this channel?',
  ...
  download_as: 'Download Channel Report',
...
var ChannelDetailsView = BaseViews.BaseListEditableItemView.extend({
  ...
  initialize: function(options) {
    ...
    if (!this.onnew) {
      var main_tree_id =
        typeof this.model.get('main_tree') == 'string'
          ? this.model.get('main_tree')
          : this.model.get('main_tree').id;
  ...
  set_background: function() {
    ...
    $('#channel_preview_wrapper').css(
      'background-image',
      "url('" + this.model.get('thumbnail_url').replace('\\', '/') + "')"
    );
  },
  events: {
  ...
  delete_channel: function() {
    ...
    dialog.dialog(
      ...
      this.get_translation('delete_warning', this.model.get('name')),
      {
...
var ChannelEditorView = BaseViews.BaseListEditableItemView.extend({
  ...
  render: function() {
    ...
    if (!this.edit) {
      this.description = new descriptionHelper.Description(
  ...
  register_changes: function() {
    ...
    if (isvalid) {
      ...
      $('#submit')
        .attr('disabled', 'disabled')
        .addClass('disabled')
        .attr('title', this.get_translation('invalid_channel'));
    }
    this.onchange(true);
  },
  ...
  download_pdf: function() {
    ...
    dialog.alert(
      this.get_translation('download_started'),
...
var DetailsView = BaseViews.BaseListEditableItemView.extend({
  ...
  render: function() {
    ...
    var original_channels = _.map(this.model.get('metadata').original_channels, function(item) {
      return item.id === self.channel_id
