...
var MESSAGES = {
  ...
  prerequisite_text:
    'Setting prerequisites will suggest relevant background or foundational content items that learners may want to visit before viewing the current concept. For learners on Kolibri, this resource will appear alongside the current content item.',
  prereq_limit:
    'Limit prerequisites for a more guided learning experience (recommended: {count, plural,\n =1 {# item}\n other {# items}})',
};
...
function PrerequisiteTree() {
  ...
  this.is_prerequisite = function(id) {
    return _.contains(this.prerequisite_list, id);
  };
  this.is_postrequisite = function(id) {
    return _.contains(this.postrequisite_list, id);
  };
  this.prerequisite_count = function() {
    return this.prerequisite_list.length;
  };
  this.get_sub_prerequisite_count = function(id) {
    return _.filter(this.ancestors, function(id2) {
      return id2 === id;
    }).length;
  };
  this.is_immediate_prerequisite = function(id) {
    return _.contains(_.keys(this.prerequisites), id);
  };
  this.is_valid_prerequisite = function(id) {
    return (
      !(this.is_prerequisite(id) && !this.is_immediate_prerequisite(id)) &&
      !this.is_postrequisite(id) &&
      !this.is_selected(id)
    );
  };
  ...
  this.update_tree = function(prerequisite_mapping, postrequisite_mapping) {
    var self = this;
    _.each(_.keys(prerequisite_mapping), function(key) {
      self.prerequisites[key] = prerequisite_mapping[key];
    });
    _.each(_.keys(postrequisite_mapping), function(key) {
      self.postrequisites[key] = postrequisite_mapping[key];
    });
  ...
  this.get_all_keys = function(jsonObject) {
    return _.chain(jsonObject)
      .map(function(item) {
        return self.get_all_keys(item);
      })
      .union(_.keys(jsonObject))
      .uniq()
      .flatten()
      .value();
  };
  ...
  this.get_postrequisite_collection_for = function(id) {
    ...
    if (this.flat_postrequisite_tree[id]) {
      collection.reset(
        this.tree_collection.filter(function(item) {
          return _.contains(self.flat_postrequisite_tree[id], item.id);
        })
      );
    }
    return collection;
  ...
  this.remove_prerequisite = function(model) {
    this.prerequisite_fetch_list = _.reject(this.prerequisite_fetch_list, function(id) {
      return id === model.id;
    });
  ...
  this.fetch_prerequisites = function() {
    ...
    return new Promise(function(resolve) {
      ...
      self.tree_collection
        .get_prerequisites(self.prerequisite_fetch_list, false)
        .then(function(nodes) {
...
var SelectedView = BasePrerequisiteView.extend({
  ...
  render: function() {
    ...
    var self = this;
    self.collection
      .get_all_fetch_simplified(PrereqTree.get_immediate_prerequisites())
...
var RelatedList = BaseViews.BaseListView.extend({
  ...
  render: function() {
    ...
    var fetchList = _.filter(this.model.get('children'), function(id) {
      return PrereqTree.is_valid_prerequisite(id);
    });
...
var BasePrerequisiteItem = BaseViews.BaseListNodeItemView.extend({
  ...
  $trs: MESSAGES,
  id: function() {
    return 'select_item_' + this.model.get('id');
  },
  initialize: function(options) {
    this.container = options.container;
    this.collection = options.collection;
