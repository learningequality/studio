...
var MESSAGES = {
  ...
  detected_import_disabled:
    'Detected items imported from another channel - fields disabled accordingly',
  title: 'Title',
  ...
  out_of_space_text:
    "You don't have enough space to save these files. Request more space under the Settings > Account page.",
...
var MetadataModalView = BaseViews.BaseModalView.extend({
  ...
  close_uploader: function(event) {
    if (
      !this.allow_edit ||
      (this.metadata_view && !this.metadata_view.check_for_changes()) ||
      !event
    ) {
      this.close();
...
var EditMetadataView = BaseViews.BaseEditableListView.extend({
  ...
  render: function() {
    ...
    var self = this;
    this.collection
      .fetch_nodes_by_ids_complete(this.collection.pluck('id'), !this.collection.has_all_data())
  ...
  load_editor: function(selected_items) {
    ...
    var has_files =
      is_individual &&
      selected_items[0].model.get('files').some(function(f) {
        return Constants.FormatPresets.find(
          preset =>
            preset.id === f.preset.name || preset.id === f.preset.id || preset.id === f.preset
        ).display;
      });
    this.$('#metadata_details_btn').css('display', selected_items.length ? 'inline-block' : 'none');
    this.$('#metadata_preview_btn').css(
      'display',
      is_individual && has_files ? 'inline-block' : 'none'
    );
    this.$('#metadata_prerequisites_btn').css(
      'display',
      is_individual && (has_files || is_exercise) ? 'inline-block' : 'none'
    );
  ...
  save_and_keep_open: function() {
    ...
    var self = this;
    if (this.validate()) {
      this.save(this.get_translation('saving'), this.save_nodes, this.save_error).then(function(
        collection
      ) {
  ...
  save_error: function(error) {
    if (error.status === 403) {
      dialog.dialog(
        this.get_translation('out_of_space'),
        this.get_translation('out_of_space_text'),
        {
      ...
      dialog.alert(
        this.get_translation('error_saving'),
        error.responseText || this.get_translation('save_failed')
      );
    }
  },
  copy_items: function() {
    if (this.collection.has_related_content()) {
      dialog.alert(
        ...
        this.get_translation('related_content_warning'),
        this.call_duplicate
      );
    } else {
      this.call_duplicate();
  ...
  save_nodes: function() {
    var sort_order =
      this.model && this.new_content ? Math.ceil(this.model.get('metadata').max_sort_order) : 0;
    var self = this;
    this.edit_list.views.forEach(function(entry) {
      var tags = entry.tags.reduce(function(list, tag) {
        return list.concat(
          JSON.stringify({ tag_name: tag, channel: State.current_channel.get('id') })
        );
      }, []);
      entry.set({
        tags: tags,
...
var EditMetadataList = BaseViews.BaseEditableListView.extend({
  ...
  validate: function() {
    var invalid_items = _.filter(this.views, function(view) {
      return view.validate();
    });
    return (
      invalid_items.length &&
      (this.selected_individual()
        ? this.selected_items[0].error || 1
        : this.get_translation('invalid_items'))
    );
  },
  ...
  update_checked: function() {
    ...
    this.views.forEach(function(view) {
      if (
        !_.contains(self.selected_items, view) &&
        view.$('.upload_item_checkbox').is(':checked')
      ) {
    ...
    if (reset) {
      ...
      this.shared_data.shared_author =
        this.shared_data.shared_author === view.model.get('author')
          ? this.shared_data.shared_author
          : null;
      this.shared_data.shared_license =
        this.shared_data.shared_license == view.model.get('license')
          ? this.shared_data.shared_license
          : 0;
      this.shared_data.shared_license_description =
        this.shared_data.shared_license_description === view.model.get('license_description')
          ? this.shared_data.shared_license_description
          : null;
      this.shared_data.all_files = this.shared_data.all_files && view.model.get('kind') !== 'topic';
      this.shared_data.all_exercises =
        this.shared_data.all_exercises && view.model.get('kind') === 'exercise';
      this.shared_data.shared_role =
        this.shared_data.shared_role == view.model.get('role_visibility')
          ? this.shared_data.shared_role
          : 0;
      this.shared_data.shared_source =
        this.shared_data.shared_source &&
        this.shared_data.shared_source.id == view.model.get('original_channel').id
          ? this.shared_data.shared_source
          : null;
      ...
      this.shared_data.shared_language =
        this.shared_data.shared_language === language || null
          ? this.shared_data.shared_language
          : 0;
      ...
      if (this.shared_data.all_exercises) {
        if (view.model.get('extra_fields')) {
          ...
          this.shared_data.shared_exercise_data = {
            mastery_model:
              exercise.mastery_model === view.model.get('extra_fields').mastery_model
                ? exercise.mastery_model
                : 0,
            m: exercise.m === view.model.get('extra_fields').m ? exercise.m : null,
            n: exercise.n === view.model.get('extra_fields').n ? exercise.n : null,
            randomize:
              exercise.randomize === view.model.get('extra_fields').randomize
                ? exercise.randomize
                : null,
          };
        } else {
...
var EditMetadataEditor = BaseViews.BaseView.extend({
  ...
  initialize: function(options) {
    ...
    this.m_value =
      this.shared_data &&
      this.shared_data.shared_exercise_data &&
      this.shared_data.shared_exercise_data.m
        ? this.shared_data.shared_exercise_data.m
        : 1;
    this.n_value =
      this.shared_data &&
      this.shared_data.shared_exercise_data &&
      this.shared_data.shared_exercise_data.n
        ? this.shared_data.shared_exercise_data.n
        : 1;
  ...
  render: function() {
    var has_files =
      this.selected_individual() &&
      _.some(this.selected_items[0].model.get('files'), function(f) {
    ...
    if (this.shared_data && this.shared_data.shared_license) {
      original_source_license = Constants.Licenses.find(
        license => license.id === parseInt(this.shared_data.shared_license, 10)
      ).license_name;
    }
    var copyright_owner =
    ...
    var all_top_level = this.new_content
      ...
      : _.all(this.selected_items, function(item) {
          return item.model.get('ancestors').length === 1;
        });
    var shared_source =
      this.shared_data &&
      this.shared_data.shared_source &&
      this.shared_data.shared_source.id !== State.current_channel.id
        ? this.shared_data.shared_source
        : null;
    if (this.allow_edit) {
      ...
      if (this.shared_data) {
        ...
        !alloriginal
          ? $('#license_select').text(stringHelper.translate(original_source_license))
        ...
        if (this.shared_data.all_exercises) {
          ...
          this.$('#mastery_custom_criterion').css(
            'display',
            this.shared_data.shared_exercise_data.mastery_model === 'm_of_n'
              ? 'inline-block'
              : 'none'
          );
        ...
        this.preview_template(
          {
            ...
            language:
              this.shared_data &&
              this.get_language(this.shared_data.shared_language, all_top_level),
    ...
    if (this.shared_data) {
      ...
      var license_name = !alloriginal
        ? original_source_license
        : this.get_license(this.shared_data.shared_license);
      this.$('#license_detail_field').text(stringHelper.translate(license_name));
      if (this.shared_data && this.shared_data.all_exercises)
        this.$('#mastery_detail_field').text(this.get_mastery_string());
      this.load_tags();
      this.$('#tag_default_detail_field').css(
        'display',
        this.shared_data.shared_tags.length ? 'none' : 'block'
      );
      this.$('#license_about').css(
        'display',
        this.shared_data.shared_license > 0 ? 'inline' : 'none'
      );
  ...
  get_language: function(language, all_top_level) {
    if (language === 0) {
      ...
      return all_top_level
        ? this.get_translation('same_as_channel')
  ...
  display_license_description: function(license_id) {
    var license =
      license_id > 0 && Constants.Licenses.find(license => license.id === parseInt(license_id, 10));
    if (license && license.is_custom) {
      this.$('#custom_license_description').css('display', 'block');
      if (this.shared_data) {
        this.$('#custom_license_description').attr(
          'placeholder',
          this.selected_individual() || this.shared_data.shared_license_description !== null
            ? this.get_translation('license_description_placeholder')
            : '---'
        );
        if (this.all_original() && this.allow_edit) {
          this.$('#custom_license_description').val(this.shared_data.shared_license_description);
        } else {
          this.$('#custom_license_description').text(
            this.shared_data.shared_license_description || this.get_translation('permissions_vary')
          );
        }
      }
    } else {
    ...
    this.$('#copyright_holder_wrapper').css(
      'display',
      license && license.copyright_holder_required ? 'block' : 'none'
    );
  },
  all_original: function() {
    return this.selected_items.every(function(item) {
  ...
  load_tags: function() {
    ...
    var self = this;
    var tags = State.Store.getters.contentTags
      .map(tag => tag.tag_name)
      .filter(tagName => self.shared_data.shared_tags.indexOf(tagName) < 0);
  ...
  load_language: function() {
    ...
    if (language === 0) {
      this.$('#select_language').val(0);
    } else if (!language) {
      this.$('#select_language').val('inherit');
    } else {
      this.$('#select_language').val(language);
    }
  },
  load_autocomplete: function() {
    ...
    this.$('.autocomplete_item').each(function(index, item) {
      ...
      if (list_name) {
        autoCompleteHelper.addAutocomplete(
          $(item),
          metadata[list_name],
          self.update_field,
          '#metadata'
  ...
  remove_tag: function(event) {
    ...
    this.shared_data.shared_tags = _.reject(this.shared_data.shared_tags, function(tag) {
      return tag === tagname;
    });
    this.load_tags();
  },
  ...
  set_selected: function() {
    ...
    var title = individual_selected
      ? this.$('#input_title')
          .val()
          .trim()
      : false;
    var description = individual_selected
      ? this.$('#input_description')
          .val()
          .trim()
      : false;
    var license =
      this.$('#license_select').is(':visible') && this.$('#license_select').val() != 0
        ? this.$('#license_select').val()
        : false;
    var copyright_holder =
      this.$('#input_license_owner').is(':visible') &&
      (individual_selected || this.$('#input_license_owner').val() !== '')
        ? self
            .$('#input_license_owner')
            .val()
            .trim()
        : false;
    var author =
      this.$('#author_field').is(':visible') &&
      (individual_selected || this.$('#author_field').val() !== '')
        ? this.$('#author_field')
            .val()
            .trim()
        : false;
    var license_description =
      this.$('#custom_license_description').is(':visible') &&
      (individual_selected || this.$('#custom_license_description').val() !== '')
        ? this.$('#custom_license_description').val()
        : false;
    var role_visibility =
      this.$('#role_select').is(':visible') && this.$('#role_select').val() != 0
        ? this.$('#role_select').val()
        : false;
    var aggregator =
      this.$('#aggregator_field').is(':visible') &&
      (individual_selected || this.$('#aggregator_field').val() !== '')
        ? this.$('#aggregator_field')
            .val()
            .trim()
        : false;
    var provider =
      this.$('#provider_field').is(':visible') &&
      (individual_selected || this.$('#provider_field').val() !== '')
        ? this.$('#provider_field')
            .val()
            .trim()
        : false;
    ...
    this.selected_items.forEach(function(view) {
      view.set_node({
        ...
        copyright_holder:
          copyright_holder === false ? view.model.get('copyright_holder') : copyright_holder,
        ...
        license_description:
          license_description === false
            ? view.model.get('license_description')
            : license_description,
        role_visibility: !role_visibility ? view.model.get('role_visibility') : role_visibility,
      });
    });
    if (this.container.show_errors) this.container.validate();
  },
...
var UploadedItem = BaseViews.BaseListEditableItemView.extend({
  ...
  set_edited: function(is_edited) {
    ...
    var edited_data = this.model.pick(
      'title',
      'description',
      'license',
      'changed',
  ...
  load_file_displays: function(formats_el) {
    ...
    this.format_view.create_thumbnail_view(
  ...
  validate: function() {
    var license = Constants.Licenses.find(
      license => license.id === parseInt(this.model.get('license'), 10)
    );
    this.error = '';
    ...
    if (!this.model.get('title')) {
      ...
      this.isoriginal &&
      this.model.get('kind') !== 'topic' &&
      license.copyright_holder_required &&
      !this.model.get('copyright_holder')
    ) {
      ...
      license &&
      license.is_custom &&
      !this.model.get('license_description')
    ) {
