...
var MoveView = BaseViews.BaseListView.extend({
  ...
  move_content: function() {
    if (this.clipboard_selected && this.collection.has_related_content()) {
      dialog.dialog(
        this.get_translation('related_content'),
        this.get_translation('related_content_warning', this.collection.length),
        {
  ...
  handle_target_selection: function(node, clipboard_selected) {
    ...
    this.$('#move_status').text(
      this.get_translation('moving_to', this.to_move_ids.length, this.target_node.get('title'))
    );
...
var MoveItem = BaseViews.BaseListNodeItemView.extend({
  ...
  initialize: function(options) {
    ...
    this.isclipboard =
      options.isclipboard || this.model.id === State.current_user.get_clipboard().id;
    this.render();
  },
  events: {
  ...
  render: function() {
    var has_descendants =
      this.model.get('metadata').resource_count < this.model.get('metadata').total_count;
    this.$el.html(
      this.template(
        {
