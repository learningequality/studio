...
var MESSAGES = {
    ...
    "default": "Default",
    "staging_deploy_prompt": "These staged changes have not been deployed to the live channel. Click <b>DEPLOY CHANNEL</b> to activate.",
    "ricecooker_no_updates_text": "Generated by {ricecookerlink} v{version} ({created}). Update content by running the Ricecooker again.",
...
var ContentItem = BaseViews.BaseWorkspaceListNodeItemView.extend({
	...
	render:function(){
		...
		var description = new descriptionHelper.Description(this.model.get("description").trim(), this.$(".description_wrapper"), 49);
		this.handle_checked();
		if(this.isSelected){
	...
	open_context_menu:function(event){
		if( event.button == 2 ) {
			...
			contextmenu.offset({
				left: (!window.isRTL)? event.pageX + 5
						: event.pageX - contextmenu.width() - 5,
				top: event.pageY + 5,
			});
			contextmenu.focus();
...
var DiffModalView = BaseViews.BaseModalView.extend({
	...
	render: function() {
		...
		State.current_channel.get_staged_diff().then(function(stats){
			self.$("#stats-table-wrapper").html(self.template({
				stats: stats,
				channel: State.current_channel.toJSON()
			}, {
