...
var AddFormulaView = BaseViews.BaseView.extend({
  ...
  render: function() {
    this.$el.html(
      this.template(
        { selector: this.selector, characters: CHARACTERS },
        {
    ...
    this.$('[data-toggle="popover"]').popover({
      html: true,
      content: this.$('#characters_' + this.selector),
    });
  },
  set_accepted_chars: function() {
    this.accepted_chars = _.chain(CHARACTERS.symbols)
      .map(function(item) {
  ...
  activate_mq: function() {
    ...
    this.parse_mathjax_characters(
      this.$('#characters_' + this.selector + ' .character_format'),
      '#character_format_',
      MQ
    );
    this.parse_mathjax_characters(
      this.$('#characters_' + this.selector + ' .character_eqn'),
      '#character_eqn_',
      MQ
    );
...
function Summernote(element, context, options) {
  ...
  this.togglePlaceholder = function(show) {
    context.$('.note-placeholder').css('display', show ? 'inline' : 'none');
  };
...
var EditorView = BaseViews.BaseView.extend({
  ...
  toggle_loading: function(isLoading) {
    if (this.editor && this.editing) {
      ...
      isLoading
        ? this.$('.editor-wrapper').addClass('disabled')
        : this.$('.editor-wrapper').removeClass('disabled');
    }
  },
      ...
      callbacks: {
        ...
        onInit: function() {
          $('.note-editor .note-btn').each(function() {
            $(this).attr(
              'data-original-title',
              self.get_translation($(this).data('original-title'))
            );
          });
        },
      },
  ...
  check_key: function(content, key, shiftkey) {
    ...
    if (shiftkey) {
      return _.contains(allowedShiftKeys, key);
    }
  ...
  add_image(file_id, filename, alt_text) {
    if (typeof file_id === 'string') {
      this.model.set(
        'files',
        this.model.get('files') ? this.model.get('files').concat(file_id) : [file_id]
      );
      alt_text = alt_text || '';
      this.model.set(
        this.edit_key,
        this.model.get(this.edit_key) + ' ![' + alt_text + '](' + filename + ')'
      );
      this.render_editor();
    }
  },
      ...
      .each(function(i, img) {
        if (!IMG_CHECK.test(img.getAttribute('src'))) {
          if (index >= files.length) {
            // Some images aren't properly added to the clipboard, so remove them to avoid errors
            $(img).remove();
          } else {
            ...
            $.post({
              ...
              error: function() {
                $(img).remove();
                dialog.alert(
                  self.get_translation('image_error_title'),
                  self.get_translation('image_error')
                );
              },
  ...
  add_formula: function(formula) {
    ...
    jax2svg.toSVG(formula).then(function(svg) {
      if (svg) {
        ...
        if (isBlank) {
          // Formulas added to blank content disable text selection (needs to be wrapped in spaces)
          self.render_editor();
        }
      } else {
        dialog.alert(
          self.get_translation('formula_error_title'),
          self.get_translation('formula_error')
        );
      }
      self.toggle_loading(false);
      self.editor.focus();
  ...
  replace_image_paths: function(content) {
    ...
    if (matches) {
      matches.forEach(function(match) {
        ...
        var replace_str =
          '/content/storage/' + filename.charAt(0) + '/' + filename.charAt(1) + '/' + filename;
    ...
    Promise.all(promises).then(function(svgs) {
      svgs.forEach(function(svg) {
        content = content.replace(
          svg.getAttribute('data-texstring'),
          '&nbsp;' + svg.outerHTML + '&nbsp;'
        );
      });
      callback(content);
...
var ExerciseEditableListView = BaseViews.BaseEditableListView.extend({
  ...
  get_default_attributes: function() {
    return {};
  }, // Default attributes to use when adding to list
  get_next_order: function() {
    if (this.collection.length > 0) {
      var max = this.collection.max(function(i) {
        return i.get('order');
      });
  ...
  switch_view_order: function(view, new_order) {
    var matches = _.filter(this.views, function(view) {
      return view.model.get('order') === new_order;
    });
...
var ExerciseEditableItemView = BaseViews.BaseListEditableItemView.extend({
  ...
  set_toolbar_open: function() {
    this.$(this.toolbar_el).html(
      this.open_toolbar_template(
        { model: this.model.attributes, undo: this.undo },
        {
          data: this.get_intl_data(),
        }
      )
...
var ExerciseView = ExerciseEditableListView.extend({
  ...
  render: function() {
    ...
    this.load_content(
      this.collection,
      this.allow_edit
        ? this.get_translation('add_question_prompt')
        : this.get_translation('no_questions_found')
    );
  },
  ...
  /*********** CONTENT PROCESSING METHODS ***********/
  validate: function() {
    return _.every(this.views, function(view) {
...
var AssessmentItemView = AssessmentItemDisplayView.extend({
  ...
  initialize: function(options) {
    _.bindAll(
      ...
      'set_toolbar_closed',
      'commit_type_change',
      'set_undo_redo_listener',
      'unset_undo_redo_listener',
      'toggle_focus',
      'set_true_false',
      'toggle_undo_redo',
      'update_hints',
  ...
  delete: function(event) {
    ...
    var self = this;
    dialog.dialog(
      this.get_translation('deleting_question'),
      this.get_translation('deleting_question_text'),
      {
  ...
  set_type: function(event) {
    ...
    if (new_type === 'true_false') {
      if (!this.model.get('answers').length) {
        ...
        dialog.dialog(
          this.get_translation('changing_question_type'),
          this.get_translation('changing_true_false'),
          {
            [this.get_translation('cancel')]: function() {},
          ...
          function() {
            self.commit_type_change(self.model.get('type'));
          }
        );
      }
    }
    ...
    // Single selection questions will set only one answer as being correct
      ...
      this.model.get('answers').where({ correct: true }).length > 1
    ) {
      dialog.dialog(
        this.get_translation('changing_question_type'),
        this.get_translation('changing_single_selection'),
        {
          [this.get_translation('cancel')]: function() {},
          [this.get_translation('change')]: function() {
    ...
    // Input questions will set all answers as being correct and remove non-numeric answers
      ...
      new_type === 'input_question' &&
      this.model.get('answers').some(function(a) {
        return a.get('correct') || numParser.test_valid_number(a.get('answer'));
      })
    ) {
      dialog.dialog(
        this.get_translation('changing_question_type'),
        this.get_translation('changing_numeric_input'),
        {
          [this.get_translation('cancel')]: function() {},
          [this.get_translation('change')]: function() {
  ...
  set_true_false: function() {
    ...
    trueFalseCollection.reset([
      { answer: this.get_translation('true'), correct: true, order: 1 },
      { answer: this.get_translation('false'), correct: false, order: 2 },
    ]);
  ...
  validate: function() {
    ...
    if (!this.model.get(this.content_field))
      this.errors.push({ error: this.get_translation('error_blank_question') });

    // Make sure answers aren't blank
    if (this.model.get('answers').findWhere({ answer: '' }))
      this.errors.push({ error: this.get_translation('error_blank_answer') });

    // Make sure hints aren't blank
    if (this.model.get('hints').findWhere({ hint: '' }))
      this.errors.push({ error: this.get_translation('error_blank_hint') });
    ...
    if (this.model.get('type') === 'input_question') {
      ...
      if (
        _.some(this.model.get('answers'), function(a) {
          return a && !NUM_REGEX.test(a.get('answer'));
        })
      ) {
...
var AssessmentItemAnswerView = ExerciseEditableItemView.extend({
  ...
  numbers_only: function() {
    return this.assessmentItem.get('type') === 'input_question';
  },
  is_single_correct: function() {
    return (
      this.assessmentItem.get('type') === 'single_selection' ||
      this.assessmentItem.get('type') === 'true_false'
    );
  },
  ...
  set_correct: function(is_correct) {
    ...
    this.$('.correct').attr(
      'title',
      is_correct ? this.get_translation('correct') : this.get_translation('incorrect')
    );
    this.propagate_changes();
    is_correct
      ? this.$('.answer_item').addClass('is_correct')
      : this.$('.answer_item').removeClass('is_correct');
  },
});
...
var HintModalView = BaseViews.BaseModalView.extend({
  ...
  closing_hints: function(event) {
    if (!this.hint_editor.validate()) {
      dialog.alert(
        this.get_translation('hint_closing_error'),
        this.get_translation('blank_item_detected')
      );
      event.stopImmediatePropagation();
      event.preventDefault();
      return false;
...
var AssessmentItemHintView = ExerciseEditableItemView.extend({
  ...
  render: function() {
    this.$el.html(
      this.template(
        { hint: this.model.toJSON(), allow_toggle: !this.isdisplay },
        {
          data: this.get_intl_data(),
        }
      )
