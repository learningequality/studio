...
var BaseAdminTab = BaseViews.BaseListView.extend({
  ...
  render: function() {
    ...
    this.$el.html(
      this.template({
        ...
        pages: Array(this.collection.state.totalPages)
          .fill()
          .map((_, i) => {
            i = i + 1;
            // should this page link render as collapsed?
            let collapsed =
              distance(currentPage, i) > visiblePageRadius && i !== 1 && i !== totalPages;
            return {
  ...
  adjustTableHeight: function(view) {
    window.requestAnimationFrame(() => {
      ...
      view
        .$('.admin_table')
        .css('max-height', `calc(100vh - 5em - ${tableTop}px - ${navHeight}px)`);
    });
  },
  check_all: function(event) {
    this.admin_list.check_all(event);
  },
...
var ChannelItem = BaseAdminItem.extend({
  ...
  set_attributes: function() {
    ...
    this.model.set(
      'can_edit',
      _.find(this.model.get('editors'), function(editor) {
        return editor.id === State.current_user.id;
      })
    );
  ...
  search_for_channel_editors: function() {
    Backbone.history.navigate(
      '/users/search/' + `${this.model.get('name')} ${this.model.get('id')}`,
      { trigger: true }
    );
  },
  download_csv: function() {
    ...
    if (!this.$('.download_csv').hasClass('disabled')) {
      this.$('.download_csv')
        .attr('title', 'Generating CSV...')
        .addClass('disabled');
      $.get({
        url: window.Urls.download_channel_content_csv(this.model.id),
        success: function() {
          dialog.alert(
            'Generating Channel CSV',
            "Channel csv generation started. You'll receive an email with the csv when it's done."
          );
          self
            .$('.download_csv')
  ...
  load_counts: function() {
    if (this.counts && this.size) {
      ...
      this.model.get_channel_counts().then(function(counts) {
        ...
        self
          .$('.counts_popover')
          .html(self.count_template({ counts: self.counts, size: self.size }));
      });
    }
  },
  ...
  make_public: function() {
    ...
    dialog.dialog(
      'Public Access',
      'Making this channel public will allow all users to view and import from this channel. Make public?',
      {
        CANCEL: function() {},
  ...
  make_private: function() {
    ...
    dialog.dialog(
      'Private Access',
      'Making this channel private will only be accessible to those with editing or view-only permissions. Make private?',
      {
        CANCEL: function() {},
  ...
  delete_channel: function() {
    ...
    dialog.dialog(
      'Deleting Channel',
      'Are you sure you want to PERMANENTLY delete this channel and all of its content? Action cannot be undone!',
      {
        CANCEL: function() {},
...
var UserTab = BaseAdminTab.extend({
  ...
  handle_checked: function() {
    this.selected_users = _.chain(this.admin_list.views)
      .where({ checked: true })
      .pluck('model')
      .value();
    if (this.selected_users.length) {
      this.$('#email_selected_users')
        .removeAttr('disabled')
        .removeClass('disabled');
      this.$('.email_button_text').text(
        'Email ' + stringHelper.format_count('User', this.selected_users.length)
      );
    } else {
      this.$('#email_selected_users')
...
var UserItem = BaseAdminItem.extend({
  ...
  deactivate_user: function() {
    ...
    dialog.dialog(
      'Deactivating User',
      'Deactivating this user will block them from using this account. Are you sure you want to continue?',
      {
        CANCEL: function() {},
  ...
  delete_user: function() {
    ...
    dialog.dialog(
      'Deleting User',
      'Are you sure you want to PERMANENTLY delete this user? Action cannot be undone!',
      {
        CANCEL: function() {},
  ...
  search_users_editable_channels: function() {
    Backbone.history.navigate(
      '/channels/search/' +
        `${this.model.get('first_name')} ${this.model.get('last_name')} ${this.model.get('email')}`,
      { trigger: true }
    );
  },
});
...
var EmailModalView = BaseViews.BaseModalView.extend({
  ...
  validate: function() {
    var has_subject = this.$('#subject_field')
      .val()
      .trim();
    var has_message = this.$('#message_area')
      .val()
      .trim();
    has_subject
      ? this.$('#subject_field').removeClass('error-field')
      : this.$('#subject_field').addClass('error-field');
  ...
  send_email: function() {
    var subject = this.$('#subject_field')
      .val()
      .trim();
