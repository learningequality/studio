<template>
  ...
  <FullscreenModal :value="dialog" :header="headerText" @input="onDialogInput">
    ...
    <VWindow v-model="step">
      <VWindowItem :value="1">
        <VContainer class="mx-0 pt-5" data-test="collection-channels-view">
          <VLayout row>
            <VFlex md12 lg10 xl8>
              <VForm ref="channelsetform">
                <VTextField v-model="name" :rules="nameRules" :label="$tr('titleLabel')" maxlength="200" counter box
              ...
              <div v-if="channelSet.secret_token">
                ...
                <CopyToken :token="channelSet.secret_token" style="max-width: max-content" />
              </div>
              ...
              <div v-else fluid>
                <KButton :text="$tr('selectChannelsHeader')" :primary="true" class="select-channels-btn"
                  data-test="button-select" @click="step++" />
                <VCard v-for="channelId in channels" :key="channelId" flat>
                  <ChannelItem :channelId="channelId">
                    <VBtn flat class="ma-0" color="primary" @click="removeChannel(channelId)">
                      {{ $tr('removeText') }}
                    </VBtn>
                  </ChannelItem>
                </VCard>
      ...
      <VWindowItem :value="2" lazy>
        <VContainer fill-height class="mx-0 pt-5" data-test="channels-selection-view">
          <VLayout row>
            <VFlex md12 lg10 xl8>
              ...
              <VContainer class="px-0">
                <Tabs showArrows slider-color="primary">
                  <VTab v-for="listType in lists" :key="listType.id">
                    {{ translateConstant(listType) }}
                  </VTab>
                  <VTabItem v-for="listType in lists" :key="listType.id" lazy>
    ...
    <MessageDialog v-model="showUnsavedDialog" :header="$tr('unsavedChangesHeader')" :text="$tr('unsavedChangesText')"
      data-test="dialog-unsaved" :data-test-visible="showUnsavedDialog">
      <template #buttons>
        <VSpacer />
    ...
    <template #bottom>
      ...
      <VSpacer />
      <KButton v-if="step === 1" class="save-finish-btn" :text="saveText" :primary="true" data-test="button-save"
        @click="save" />
      <KButton v-else class="save-finish-btn" :text="$tr('finish')" :primary="true" data-test="button-finish"
        @click="finish" />
    </template>
  </FullscreenModal>
