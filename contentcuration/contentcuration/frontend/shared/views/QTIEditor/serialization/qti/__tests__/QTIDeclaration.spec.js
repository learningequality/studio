...
import { SIMPLE_MAPPING_XML } from './declarations/fixtures.js';
import {
...
describe('QTIDeclaration.getScoringRule', () => {
  it('prefers the correct response over a capability registered before it', () => {
    const d = new QTIDeclaration({ identifier: 'RESPONSE', baseType: 'identifier' });
    d.registerCapability(CAPABILITY.MAPPING, {
      get: () => null,
      getXML: () => null,
      getScoringRule: () => buildXmlNode({ tag: 'mapping-rule' }),
    });
    const cr = new CorrectResponse(['ChoiceA'], d);
    expect(d.getScoringRule('RAW_SCORE')).toEqual(cr.getScoringRule('RAW_SCORE'));
  });

  it('returns null when no capability can score', () => {
    const d = QTIDeclaration.fromXML(parseXML(SIMPLE_MAPPING_XML));
