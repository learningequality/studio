...
describe('buildXML()', () => {
  ...
  describe('interaction attributes', () => {
    it('emits the response identifier, shuffle, and one association per pair', () => {
  ...
  describe('choice pool', () => {
    it('merges pairs and distractors into one pool of singly-used choices', () => {
      const root = parseXmlString(build(baseState).bodyXml);
    ...
    it('keeps the first appearance of an id and reassigns the later choice reusing it', () => {
      const pairs = [
        baseState.pairs[0],
        [
      ...
      const [capulet] = choicesOf(root).filter(el => el.textContent === 'Capulet');
      const [antonio] = choicesOf(root).filter(el => el.textContent === 'Antonio');
      expect(antonio.getAttribute('identifier')).toBe('choice_aaa11111');
  ...
  describe('response declaration', () => {
    ...
    it('takes cardinality and base-type from the declaration schema', () => {
      const decl = parseXmlString(build(baseState).responseDeclarations[0]);
