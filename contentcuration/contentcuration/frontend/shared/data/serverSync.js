 ...
 */
function syncChanges(syncAllChanges, useWebsocket = false) {
    ...
    try {
      ...
      if (!useWebsocket) {
        const unAppliedChanges = await db[CHANGES_TABLE].orderBy('server_rev')
          ...
          .toArray();
        requestPayload.unapplied_revs = unAppliedChanges.map(c => c.server_rev).filter(Boolean);
      }
      ...
      let response;
      if (useWebsocket) {
        response = await socket.send(
          JSON.stringify({
            payload: requestPayload,
          })
        );
      } else {
        response = await client.post(urls['sync'](), requestPayload);
      }
...
export function debouncedSyncChanges(flush = false, syncAll = false, useWebsocket = false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (flush) {
      ...
      doSyncChanges(syncAll, useWebsocket);
    } else {
      // Otherwise update the timeout to this invocation.
      syncDebounceTimer = setTimeout(() => doSyncChanges(syncAll, useWebsocket), syncDebounceWait);
    }
  });
}
...
export function syncOnChanges() {
  vueInstance.$watch(
    () => changeRevs.length,
    () => debouncedSyncChanges(false, false, true)
  );
}
