<template>
  ...
  <FullscreenModal v-model="dialog" color="black">
    <template #close>
    ...
    <VContainer v-else classs="ml-5">
      <VLayout>
        <VSpacer />
        <UserActionsDropdown :userId="userId" />
      </VLayout>
      <h1>{{ user.name }}</h1>
      ...
      <DetailsRow
        label="Last active"
        :text="user.last_login? $formatRelative(user.last_login, { now: new Date() }) : 'N/A'"
      />
      <DetailsRow
  ...
  export default {
    ...
    computed: {
      ...
      space: {
        ...
        set(value) {
          this.updateUser({ id: this.userId, disk_space: value });
        },
      },
      backLink() {
        return {
