    ...
    selectedDuration: {
      get() {
        if (this.kind === 'audio' || this.kind === 'video') {
          ...
          console.log('!!! SET selectedDuration not audio or video', this.value);
          return this.value.model
            ? this.findDurationObject(this.value.model).id
            : 'exactTime' || {};
        }
      },
      set(duration) {
        ...
        // if (this.kind === 'audio' || this.kind === 'video') {
        console.log('!!! this.durationValue is ', this.durationValue);
        if (this.durationValue === 'exactTime') {
          this.handleInput({
            suggested_duration: 849, //TODO: temporary - remove!
            completion_criteria: {
    ...
    selectedCompletion: {
      ...
      set(value) {
        ...
        if (this.completionValue === CompletionOptionsMap.allContent) {
          console.log('!!! yes, all content viewed');
          return this.handleInput({ model: CompletionCriteriaModels.PAGES, threshold: '100%' });
        }

        if (this.completionValue === CompletionOptionsMap.completeDuration) {
          console.log('!!! yes, complete duration, wait for duration to be set');
        }
      },
    },
