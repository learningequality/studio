...
class SettingsTestCase(BaseAPITestCase):
    def test_username_change(self):
        data = json.dumps({"first_name": 'New firstname',
                           "last_name": 'New lastname',
                           })
        request = self.create_post_request(reverse_lazy("update_user_full_name"), data=data, content_type='application/json')
        response = UsernameChangeView.as_view()(request)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(User.objects.filter(email=self.user.email, first_name='New firstname', last_name='New lastname').exists())

    def test_delete_account_invalid(self):
        data = json.dumps({"email": 'not the right email'})
        request = self.create_post_request(reverse_lazy("delete_user_account"), data=data, content_type='application/json')
        try:
    ...
    def test_delete_account(self):
        ...
        self.create_post_request(reverse_lazy("delete_user_account"), data=data, content_type='application/json')
