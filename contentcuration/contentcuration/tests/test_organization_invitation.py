...
class OrganizationInvitationTestCase(BaseAPITestCase):
    ...
    def test_sending_an_invitation_emails_the_invitee(self):
    ...
    def test_invitation_email_links_to_the_organizations_page_for_an_active_user(self):
        User.objects.create(
            email="active-invitee@example.com", first_name="Active", is_active=True
        )

        self.send_invitation("active-invitee@example.com", "edit")

        domain = Site.objects.get_current().domain
        expected_link = "https://{}{}#/my-organizations".format(
            domain, reverse("channels")
        )
        self.assertIn(expected_link, mail.outbox[0].body)

    def test_invitation_email_has_no_organizations_fragment_for_a_new_user(self):
        self.send_invitation("brand-new-invitee@example.com", "edit")

        self.assertNotIn("#/my-organizations", mail.outbox[0].body)
        domain = Site.objects.get_current().domain
        expected_link = "https://{}{}".format(
            domain,
            reverse(
                "accept_invitation_and_registration",
                args=["brand-new-invitee@example.com"],
            ),
        )
        self.assertIn(expected_link, mail.outbox[0].body)
