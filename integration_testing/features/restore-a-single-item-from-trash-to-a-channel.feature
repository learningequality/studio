Feature: Restore a single item from trash to a channel

	Background: 
		Given That I am signed in to Studio
			And I am on the <channel> editing page

	Scenario: Restore a single item from trash to a channel
		...
		Then I see the *Moving 1 item to <channel>* notification
		When I click the arrow button to open the <channel> topic tree
			And I select the <topic> topic
		Then I see the *Moving 1 item to <topic>* notification
